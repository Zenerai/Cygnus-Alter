...
public class ArchiveMovement_2D : MonoBehaviour
{
    //Long story short, this is an old file for dummy player, do not use!!
    ...
    {
        // This is unused code left as example.
        /*if (displayCrouchSprite)
        {
            sr.sprite = greenArrow;
            return;
        }
        if (isWalled)
        {
        ...
        if (!isGrounded && rb.velocity.y > 0.1f) //Jumping up
        {
            sr.sprite = redArrow;
            return;
        }
        ...
        {
            sr.sprite = yellowDoubleArrow;
            return;
        }
        sr.sprite = blueArrow; //Default: Regular/Idle/Move/Fall (blank ignored)
        */
